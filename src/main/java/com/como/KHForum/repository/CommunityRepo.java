...
public interface CommunityRepo extends JpaRepository<Community, Long> {
    ...
    @Query(value = "select uc.* from kh_forum.communities as uc left join kh_forum.user_communties as c " +
                   "on uc.id = c.community_id where c.user_id = :user_id", nativeQuery = true)
